...
##########################################################################
	...
	def shutdown_hook(self):
		...
		rospy.signal_shutdown("Shutdown Command Recieved.")
...
##########################################################################
	...
	def load_goals(self):
		...
		Dir is the direction the robot will approach the waypoints from.
		'B' would be going to the waypoint backwards. 'F' would 
		be forwards. 
		'''
		...
		while i < len(loaded_x):
			...
			self.dir=np.append(self.dir,loaded_dir[i])
			i+=1
	...
	def print_goals(self):
		'''
		Prints ALL waypoint goals' x and y coordinates, forwards, backwards.
		'''
		rospy.loginfo("List of goals:")
		rospy.loginfo("X:\t " + str(self.x))
		rospy.loginfo("Y:\t " + str(self.y))
		rospy.loginfo("Direction:\t"+ str(self.dir).upper())
	...
	def print_goal(self):
		...
		rospy.loginfo("Drive direction: " + str(self.dir[self.active_goal]).upper())
...
##########################################################################
	...
	def print_goal_and_pose(self):
		...
		rospy.loginfo("The direction is:")
		rospy.loginfo(str(self.dir[self.active_goal]).upper())
	...
	def velocity_controller(self):
		...
		required_angle = self.angle_wrap(math.atan2((self.y[self.active_goal] - self.position_y), (self.x[self.active_goal] - self.position_x)))
                if (str(self.dir[self.active_goal]).upper() == "B"):
                    # Add 180 degrees to orientation
                    required_angle+=3.14
                    required_angle = self.angle_wrap(required_angle)
                    if not self.set_direction:
                        # Backwards linear velocity
                        self.linear_spd = -self.linear_spd
		...
		if not self.has_arrived_xy():	
			...
			print("Goal is: x: ", self.x[self.active_goal], "y", self.y[self.active_goal])
